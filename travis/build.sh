...
elif [[ -z "$TRAVIS_TAG" && "$TRAVIS_BRANCH" == "mPower-Android" ]]; then  # non-tag commits to master branch
    bundle exec fastlane test
    bundle exec fastlane assemble
elif [[ -z "$TRAVIS_TAG" && "$TRAVIS_BRANCH" =~ ^mPower-Android-.* ]]; then # non-tag commits to stable branches
